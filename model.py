...
class BiLSTM_CRF(object):
    def __init__(self, args, embeddings, tag2label, vocab, outputdir, config):
        ...
        self.vocab = vocab
        self.model_path = os.path.join(outputdir, 'checkpoints')
        if not os.path.exists(self.model_path):
            os.mkdir(self.model_path)
        self.result_path = os.path.join(outputdir, 'results')
    ...
    def run_one_epoch(self, sess, train, dev, tag2label, epoch, saver):
        ...
        batches = batch_yield(train, self.batch_size, self.vocab, self.tag2label, shuffle=True)
        for step, (seqs, labels) in enumerate(batches):
            step_num = epoch * num_batches + step + 1
            feed_dict, _ = self.get_feed_dict(seqs, labels, self.lr, self.dropout_keep_prob)
            _, loss_train, step_num_ = sess.run([self.train_op, self.loss, self.global_step],
                                                         feed_dict=feed_dict)
            print('\r{} epoch {}, step {}, loss: {:.4}, global_step: {}'.format(start_time, epoch + 1, step + 1, loss_train, step_num), end='')
    ...
    def predict(self, model_path, test):
